...
void rotate(unsigned long calibrateDuration, float deltaAngle, float tol, float minRange, float maxRange, uint8_t minPower, uint8_t maxPower, float k_p, float k_i, float k_d) {
  ...
  myICM.getAGMT();
  while (abs(currentAngle - deltaAngle) > tol) {
    float dt = (currentMillis - previousMillis) / 1000.0;
    float speed = myICM.gyrZ() - calibration;
    currentAngle += speed * dt;

    float error = currentAngle - deltaAngle;
    integrator += error * dt;
    float pid = k_p * error + k_i * integrator + k_d * speed;
...
void drive(uint16_t deltaDistance, int tol, float minRange, float maxRange, uint8_t minPower, uint8_t maxPower, float k_p, float k_i, float k_d) {
  distanceSensor1.startRanging();
  
  unsigned long lastMillis, previousMillis, currentMillis;
  uint16_t lastDistance, currentDistance;
  float integrator = 0;
  float speed = 0;
  ...
  lastDistance = currentDistance = distanceSensor1.getDistance();
  distanceSensor1.clearInterrupt();

  int32_t target = currentDistance - deltaDistance;

  while (abs(currentDistance - target) > tol) {
    currentMillis = millis();
    float dt = currentMillis - previousMillis;

    if (distanceSensor1.checkForDataReady()) {
      currentDistance = distanceSensor1.getDistance();
      distanceSensor1.clearInterrupt();
      speed = (float) (currentDistance - lastDistance) / (currentMillis - lastMillis);
    ...
    float error = currentDistance - target;
    integrator += error * dt;
    float pid = k_p * error + k_i * integrator + k_d * speed;

    uint8_t motor = (abs(pid) - minRange) * (maxPower - minPower) / (maxRange - minRange) + minPower;
    if (pid < 0) {
      motorWrite(0, motor, 0, motor);
    } else {
      motorWrite(motor, 0, motor, 0);
    }
  }
...
void handle_command() {
  ...
  switch (cmd_type) {
    ...
    /**
      * ROTATE:calibrateDuration|deltaAngle|tolerance|minPid|maxPid|minMotor|maxMotor|k_p|k_i|k_d
      * 
      */
    case ROTATE:
      success = robot_cmd.get_next_value(duration)
      ...
      ;
      if (!success) return;
      rotate(duration, fval2, fval3, fval4, fval5, ival1, ival2, fval6, fval7, fval8);
      break;
    
    /**
      * DRIVE:deltaDistance|tolerance|minPid|maxPid|minMotor|maxMotor|k_p|k_i|k_d
      * 
      */
    case DRIVE:
      success = true
      ...
      ;
      if (!success) return;
      drive(ival4, ival3, fval4, fval5, ival1, ival2, fval6, fval7, fval8);
      break;
